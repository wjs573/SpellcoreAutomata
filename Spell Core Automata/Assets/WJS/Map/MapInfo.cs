...
namespace WJS
{
    public class MapInfo
    {
        ...
        public bool CanUnitPlacedHere(Vector3 pos, float radius, MoveType moveType)
        {
            ...
            return !CommonScripts.CircleHitRects(new Vector2(pos.x, pos.z), radius, collisionRects);
        }
        ...
        public Vector3 GetCentralPosForCharacter(RectInt range, float chaRadius = 0.00f, MoveType moveType = MoveType.ground)
        {
            // 计算给定范围的中心坐标
            int centerX = range.x + range.width / 2;
            int centerY = range.y + range.height / 2;
            ...
            // 检查中心点是否适合放置角色
            if (CanUnitPlacedHere(centralPos, chaRadius, moveType))
            {
                return centralPos;
            }

            // 如果中心点不适合，则返回 Vector3.zero
            return Vector3.zero;
        }
        ...
        ///</summary>
        public float GetNearestVerticalBlock(Vector3 pivot, float dir, float radius, MoveType moveType, bool ignoreBorder, GameObject gameObject)
        {
        ...
        ///</summary>
        public float GetNearestHorizontalBlock(Vector3 pivot, float dir, float radius, MoveType moveType, bool ignoreBorder, GameObject gameObject)
        {
        ...
        ///</summary>
        public MapTargetPosInfo FixTargetPosition(Vector3 pivot, float radius, Vector3 targetPos, MoveType moveType, bool ignoreBorder, GameObject gameObject)
        {
            float xDir = targetPos.x - pivot.x;
            float zDir = targetPos.z - pivot.z;
            float bestX = GetNearestVerticalBlock(pivot, xDir, radius, moveType, ignoreBorder, gameObject);
            float bestZ = GetNearestHorizontalBlock(pivot, zDir, radius, moveType, ignoreBorder, gameObject);
        ...
        public Vector3 GetNearestValidPosition(Vector3 currentPos, float unitRadius, MoveType moveType)
        {
            // 首先，检查当前位置是否已经在地图内
            if (Border.Contains(new Vector2(currentPos.x, currentPos.z)))
            {
                return currentPos;
            }

            // 如果单位在地图外，找到最近的边界点
            float nearestX = Mathf.Clamp(currentPos.x, Border.xMin, Border.xMax);
            float nearestZ = Mathf.Clamp(currentPos.z, Border.yMin, Border.yMax);

            // 确保该点可通行
            Vector3 nearestValidPos = new Vector3(nearestX, currentPos.y, nearestZ);
            if (CanUnitPlacedHere(nearestValidPos, unitRadius, moveType))
            {
                return nearestValidPos;
            }
            return GetRandomPosForCharacter(new RectInt(0, 0, MapWidth(), MapHeight()), unitRadius, moveType);
        }
    }
}
