...
public class EventManager<T> where T : Delegate
{
    ...
    private T CreateHandlerForTimelineModel(TimelineModel timelineModel)
    {
        ...
        else if (typeof(T) == typeof(LaserOnCreate))
        {
            LaserOnCreate laserOnCreateDelegate = new LaserOnCreate((laser) =>
            {
                TimelineObj newTimelineobj = new TimelineObj(timelineModel, null, new object[] { EventManagerType.LaserOnCreate, laser });
                DelegateCastTimelineObj(newTimelineobj);
            });
            return (T)(Delegate)laserOnCreateDelegate;
        }
        else if (typeof(T) == typeof(LaserOnHit))
        {
            LaserOnHit laserOnHitDelegate = new LaserOnHit((laser, target) =>
            {
                TimelineObj newTimelineobj = new TimelineObj(timelineModel, null, new object[] { EventManagerType.LaserOnHit, laser, target });
                DelegateCastTimelineObj(newTimelineobj);
            });
            return (T)(Delegate)laserOnHitDelegate;
        }
        else if (typeof(T) == typeof(LaserOnRemoved))
        {
            LaserOnRemoved laserOnRemovedDelegate = new LaserOnRemoved((laser) =>
            {
                TimelineObj newTimelineobj = new TimelineObj(timelineModel, null, new object[] { EventManagerType.LaserOnRemoved, laser });
                DelegateCastTimelineObj(newTimelineobj);
            });
            return (T)(Delegate)laserOnRemovedDelegate;
        }
    ...
    private void DelegateCastTimelineObj(TimelineObj timelineObj)
    {
        ...
        switch (eventType)
        {
            ...
            case EventManagerType.LaserOnCreate:
                HandleLaserOnCreate(timelineObj);
                break;

            case EventManagerType.LaserOnHit:
                HandleLaserOnHit(timelineObj);
                break;

            case EventManagerType.LaserOnRemoved:
                HandleLaserOnRemoved(timelineObj);
                break;
    ...
    private void CreateTimelineByBullet(GameObject bullet, GameObject target, TimelineObj timelineObj)
    {
        ...
        if (target != null)
        {
            ...
            {
                DesignerTables.DataEnhancedEffect.ModifyParameterOfType<BulletLauncher>(timelineObj.model.nodes[i].eveParams, (BulletLauncher value) =>
                {
    ...
    private ChaState CreateBulletLauncherCharacter(GameObject bullet)
    {
        ...
        //添加定时销毁buff
        AddBuffInfo addBuffInfo = new AddBuffInfo(DesignerTables.Buff.data["ScheduledDead"], DelegateCharacater, DelegateCharacater, 1, 3f);
    ...
    private void CreateTimelineByAoe(GameObject aoe, TimelineObj timelineObj)
    {
        ...
        delegateCharacaterState.SetImmuneTime(10f);
        //添加定时销毁buff
        AddBuffInfo addBuffInfo = new AddBuffInfo(DesignerTables.Buff.data["ScheduledDead"], DelegateCharacater, DelegateCharacater, 1, 0.1f);
        delegateCharacaterState.AddBuff(addBuffInfo);
    ...
    private void CreateTimelineByLaser(GameObject laser, TimelineObj timelineObj)
    {
        LaserState laserState = laser.GetComponent<LaserState>();
        LineRenderer lineRenderer = laserState.Laser;
        ChaState chaState = laserState.caster.GetComponent<ChaState>();
        Vector3 position = new Vector3(lineRenderer.GetPosition(1).x, 0, lineRenderer.GetPosition(1).z);
        // 创建临时施法者
        GameObject DelegateCharacater = SceneVariants.CreateCharacter("DelegateCharacater", chaState.side, position, chaState.property, 0f);
        ChaState delegateCharacaterState = DelegateCharacater.GetComponent<ChaState>();
        //关闭碰撞体
        DelegateCharacater.GetComponent<CapsuleCollider>().enabled = false;
        delegateCharacaterState.SetImmuneTime(10f);
        ...
        timelineObj.values.Add("Caster", laserState.caster);
        timelineObj.caster = DelegateCharacater;
        SceneVariants.CreateTimeline(timelineObj);
    }

    private void HandleLaserOnCreate(TimelineObj timelineObj)
    {
        var laser = (GameObject)timelineObj.param[1];

        _callBackParams = new CallBackParams();
        _callBackParams.laser = laser;

        if (_condition != null)
        {
            if (_condition(_callBackParams, _conditionParams))
            {
                CreateTimelineByLaser(laser, timelineObj);
            }
        }
        else
        {
    ...
    private void HandleLaserOnHit(TimelineObj timelineObj)
    {
        var laser = (GameObject)timelineObj.param[1];
        var target = (GameObject)timelineObj.param[2];
    ...
    private void HandleLaserOnRemoved(TimelineObj timelineObj)
    {
        var laser = (GameObject)timelineObj.param[1];
        // 处理 LaserOnRemoved 事件的逻辑
        // 处理 LaserOnHit 事件的逻辑
        _callBackParams = new CallBackParams();
        _callBackParams.laser = laser;
