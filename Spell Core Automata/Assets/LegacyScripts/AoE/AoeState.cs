...
public class AoeState : MonoBehaviour
{
    ...
    public float innerRadius; // 内半径，用于特殊的扇形-扇环
    public float startAngle; // 开始角度，用于扇形 AoE
    public float rotationAngle;// 旋转角度，用于扇形AoE
    public float zLength; // 用于矩形 AoE 特指z轴上的长度
    public float xLength; // 用于矩形 AoE 特指x轴上的长度

    public bool isDieWithParent = false;
    ...
    public void InitByAoeLauncher(AoeLauncher aoe)
    {
        ...
        if (aoe.model.prefab != "")
        {
            ...
            this.GetComponentInChildren<ViewContainer>().GetSkillSize = ()=> this.radius;
        }
    ...
    void OnDrawGizmos()
    {
        Gizmos.color = Color.blue; // 设置绘制的颜色

        // 使用DrawWireSphere绘制一个圆
