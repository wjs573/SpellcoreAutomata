...
{
    public int ordinary_damage;
    public int true_damage;

    public static Damage GetDamageByString(string damageType)
    {
        return damageType switch
        {
            "" => new Damage(1, 1),
            "无属性伤害" => new Damage(1),
            "真实属性伤害" => new Damage(0, 1),
            _ => new Damage(0),
        };
    }

    public Damage(int ordinary_damage, int true_damage = 0)
    {
        this.ordinary_damage = ordinary_damage;
    ...
    {
        return (asHeal == false) ?
            (Mathf.Max(0, ordinary_damage) + Mathf.Max(0, true_damage)) :
            (Mathf.Min(0, ordinary_damage) + Mathf.Min(0, true_damage));
    }

    public static Damage operator +(Damage a, Damage b)
    {
        return new Damage(
            a.ordinary_damage + b.ordinary_damage,
            a.true_damage + b.true_damage
        );
    }
    ...
    {
        return new Damage(
            Mathf.RoundToInt(a.ordinary_damage * b),
            Mathf.RoundToInt(a.true_damage * b)
        );
    }
    ...
    {
        return new Damage(
            Mathf.RoundToInt(a.ordinary_damage * b.ordinary_damage),
            Mathf.RoundToInt(a.true_damage * b.true_damage)
        );
    }
}
...
public enum DamageInfoTag
{
    ...
    directHeal = 10,    //直接治疗
    periodHeal = 11,    //间歇性治疗  
    NoNeedPopText = 9998,     //不需要pop text
