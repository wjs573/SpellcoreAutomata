...
public struct BuffModel
{
    ...
    )
    {
        ...
        this.onOccur = (onOccur == "") ? null : new EventManager<BuffOnOccur>(DesignerScripts.Buff.onOccurFunc[onOccur]);
        this.onOccurParams = occurParam;
        this.onRemoved = (onRemoved == "") ? null : new EventManager<BuffOnRemoved>(DesignerScripts.Buff.onRemovedFunc[onRemoved]);
        this.onRemovedParams = removedParam;
        this.onTick = (onTick == "") ? null : new EventManager<BuffOnTick>(DesignerScripts.Buff.onTickFunc[onTick]);
        this.onTickParams = tickParam;
        this.onCast = (onCast == "") ? null : new EventManager<BuffOnCast>(DesignerScripts.Buff.onCastFunc[onCast]);
        this.onCastParams = castParam;
        this.onHit = (onHit == "") ? null : new EventManager<BuffOnHit>(DesignerScripts.Buff.onHitFunc[onHit]);
        this.onHitParams = hitParam;
        this.onBeHurt = (beHurt == "") ? null : new EventManager<BuffOnBeHurt>(DesignerScripts.Buff.beHurtFunc[beHurt]);
        this.onBeHurtParams = hurtParam;
        this.onKill = (onKill == "") ? null : new EventManager<BuffOnKill>(DesignerScripts.Buff.onKillFunc[onKill]);
        this.onKillParams = killParam;
        this.onBeKilled = (beKilled == "") ? null : new EventManager<BuffOnBeKilled>(DesignerScripts.Buff.beKilledFunc[beKilled]);
