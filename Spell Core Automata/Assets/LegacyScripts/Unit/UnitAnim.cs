...
public class UnitAnim : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Transform viewContainer = transform.Find("ViewContainer");
        if (viewContainer == null)
        {
            Debug.LogError("ViewContainer not found");
            return;
        }

        // 获取ViewContainer下所有模型的Transform
        modelTransforms = new List<Transform>();
        foreach (Transform child in viewContainer)
        {
            modelTransforms.Add(child);
        }
    }
    ...
    public void Play(string animName)
    {
        ...
        if (animInfo == null)
        {
            //Debug.Log($"Error: animName '{animName}' animInfo is null.");
            return;
        }

        if (!animInfo.ContainsKey(animName))
        {
            //Debug.Log($"Error: animName '{animName}' not found in animInfo.");
            return;
        }

        if (animator == null)
        {
            //Debug.Log($"Error: animName '{animName}' animator is null.");
            return;
        }

        if (playingAnim != null && playingAnim.key == animName)
        {
            //Debug.Log($"Info: Animation '{animName}' is already playing.");
            return;  //已经在播放了
        }
        ...
        {
            //Debug.Log($"Info: Animation '{animName}' has lower priority than current animation.");
            return;   //优先级不够不放
        }
        ...
        {
            Debug.Log($"Error: Exception occurred while trying to play animation '{animName}'. Exception: {ex.Message}");
        }
    }
