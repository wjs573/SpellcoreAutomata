...
namespace FIMSpace.BonesStimulation
{
    public partial class BonesStimulator
    {
        ...
        void _Gizmos_DrawColliders()
        {
            ...
            if (UseCollisions)
            {
                ...
                for (int i = 0; i < Bones.Count; i++)
                {
                    ...
                    if (Application.isPlaying) Gizmos.DrawWireSphere(pos, Bones[i].GetCollisionRadiusScaled());
                    else Gizmos.DrawWireSphere(pos, GetColliderSphereRadiusFor(i, Bones.Count+1));
                    Gizmos.color = sphColor;
                }
                ...
                if (Bones.Count > 0)
                    if (GetLastTransform() != null)
                        if (!Application.isPlaying)
                        {
                            ...
                            if (Application.isPlaying) Gizmos.DrawWireSphere(pos, Bones[Bones.Count - 1].GetCollisionRadiusScaled());
                            else Gizmos.DrawWireSphere(pos, GetColliderSphereRadiusFor(Bones.Count, Bones.Count+1));
                        }
        ...
        /// </summary>
        protected float GetColliderSphereRadiusFor(int i, float bonesCount)
        {
            float step = 0f;
            if (Bones.Count > 1) step = 1f / (bonesCount);
        ...
        public void RefreshCollidersRadius()
        {
            for (int i = 0; i < Bones.Count; i++)
                Bones[i].CollisionRadius = GetColliderSphereRadiusFor(i, Bones.Count);
        }
