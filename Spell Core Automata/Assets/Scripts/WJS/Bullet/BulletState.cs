...
namespace WJS
{
    ...
    public class BulletState : MonoBehaviour
    {
        ...
        public void InitByBulletLauncher(BulletLauncher bullet, GameObject[] targets)
        {
            ...
            {
                // 加载子弹预制件资源
                GameObject bulletPrefabResource = AssetDatabaseManager.Instance.GetPrefab(bullet.model.prefab);
                GameObject bulletEffect = null;
                // 检查预制件是否为空
                if (bulletPrefabResource == null)
                {
                    Debug.LogError("The bullet prefab '" + bullet.model.prefab + "' could not be found in AssetDatabase. Please check the path and file name.");
                }
                else
                {
                    // 预制件非空，安全地实例化预制件
