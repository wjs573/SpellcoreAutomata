...
public class ChaState : MonoBehaviour
{
    ...
    {
        get { return parentChaState == null ? this : parentChaState; }
    }
        ...
        else
        {
            ...
            for (int i = 0; i < toAddBuff.model.tags.Length; i++)
            {
                if (toAddBuff.model.tags[i] == "debuff" || toAddBuff.model.tags[i] == "Control" || toAddBuff.model.tags[i] == "Pop")
                {
                    ...
                    {
                        PopTextManager.Instance.PopUpStringOnCharacter(toAddBuff.carrier, "中毒", Color.green);
                    }
                    break;
                }
            }
    ...
    public List<BuffObj> GetBuffById(string id, List<GameObject> caster = null)
    {
        ...
        {
            if (State.buffs[i].model.id == id && (caster == null || caster.Count <= 0 || caster.Contains(State.buffs[i].caster) == true))
            {
                res.Add(State.buffs[i]);
            }
        }
        return res;
    }
    ...
    public SkillObj GetSkillById(string id)
    {
        ...
        {
            if (State.skills[i].model.id == id)
            {
                return State.skills[i];
            }
        }
        return null;
    }
    ...
    public bool CastSkill(string id)
    {
        ...
        if (State.resource.Enough(skillObj.model.condition) == true)
        {
            ...
            for (int i = 0; i < State.buffs.Count; i++)
            {
                if (State.buffs[i].model.onCast != null)
                {
                    timeline = State.buffs[i].model.onCast.Invoke<TimelineObj>(State.buffs[i], skillObj, timeline);
                }
            }
            if (timeline != null)
            {
                //法宝释放技能 消耗父亲的资源
                State.ModResource(-1 * skillObj.model.cost);
        ...
        {
            //每100点冷却速率可以降低一半冷却时间
            skillObj.cooldown = skillObj.model.cooldown / (State.property.cd_speed / 100 + 1);
        }
        else
        {
