...
public class TimelineObj
{
    ...
    public TimelineType timelineType = TimelineType.Character;
    ...
    public GameObject GetCaster()
    {
        if (_caster == null)
        {
            if (values != null && values.ContainsKey("Caster"))
            {
                return (GameObject)values["Caster"];
            }
        }
        return _caster;
    }

    public void SetCaster(GameObject value)
    {
        _caster = value;
    }
    ...
    public TimelineObj(TimelineModel model, GameObject caster, object[] param,
        TimelineType timelineType = TimelineType.Character, GameObject weapon = null)
    {
        this.model = model;
        this.caster = caster;
...
public struct TimelineModel
{
    ...
    public void ResetEventManager()
    {
        ...
        {
            DataEnhancedEffect.ModifyParameterOfType<BulletLauncher>(nodes[i].eveParams, (BulletLauncher value) =>
            {
                value.model.ResetEvent();
                return value;
            });
            DataEnhancedEffect.ModifyParameterOfType<AoeLauncher>(nodes[i].eveParams, (AoeLauncher value) =>
            {
                value.model.ResetEvent();
                return value;
            });
            DataEnhancedEffect.ModifyParameterOfType<LaserLauncher>(nodes[i].eveParams, (LaserLauncher value) =>
            {
                value.model.ResetEvent();
                return value;
            });
    ...
    public TimelineModel Clone()
    {
        ...
        for (int i = 0; i < this.nodes.Length; i++)
        {
            ...
            if (this.nodes[i].eveParams != null)
            {
                ...
                {
                    if (param is BulletLauncher)
                    {
                        // 如果param是BulletLauncher类型，进行克隆
                        BulletLauncher bulletLauncher = (BulletLauncher)param;
                    ...
                    else if(param is AoeLauncher)
                    {
                        AoeLauncher aoeLauncher = (AoeLauncher)param;
...
public struct TimelineNode : IComparable<TimelineNode>
{
    ...
    public TimelineNode(float time, string doEve, params object[] eveArg)
    {
        ...
        this.eveParams = eveArg != null ? eveArg.ToArray() : null; // 创建一个新的数组
        this.loopTimes = 1;
