    ...
    public class AoE
    {
        ...
        private static void FreezesEnemyInRange(GameObject aoe)
        {
            ...
            for (int i = 0; i < aoeState.characterInRange.Count; i++)
            {
                ...
                //如果是存在 且 不是法宝（召唤物） 且 活着 且 属于敌军
                if (chaState != null && !chaState.hasParent && !chaState.dead && chaState.side != casterState.side)
                {
        ...
        private static void DoDamageOnRemoved(GameObject aoe)
        {
            ...
            if (aoeState.caster)
            {
                ChaState ccs = aoeState.caster.GetComponent<ChaState>().State;
                if (ccs) side = ccs.side;
            }
